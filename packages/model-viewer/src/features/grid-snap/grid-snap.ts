...
Raycaster.prototype.intersectOctreeObject = function(object, recursive) {
  var intersects, octreeObject, facesAll, facesSearch;
...
function GridSnap(
    scene, renderer, camera, mesh, radius, hoverMesh, markerMesh) {
  ...
  if (this.hoverMesh) {
    scene.add(this.hoverMesh);  // Since clickMeshes are not adde by default,
                                // this shouldn't be either
    this.hoverMesh.renderOrder =
        999;  // hoverMesh will render above marker objects. TODO: improve this
              // so it doesn't render above all objects in scene
    this.hoverMesh.visible = false
  }

  this.octree = new Octree({undeferred: true});

  // TODO: support more than 1 mesh. Can probably use children?
  if (this.intersectMesh.isMesh) {
    this.octree.add(this.intersectMesh, {useVertices: true});
  }
}

GridSnap.prototype.getNearestVertexFromEvent =
    function(event) {
  ...
  var intersects =
      this.raycaster.intersectOctreeObject(this.intersectMesh, true);

  console.log('intersects', intersects);
  if (intersects.length > 0) {
    var nearestPoint = intersects[0].point;
    console.log('nearestPoint', nearestPoint, this.radius);
    var closestVertex =
        this.octree.findClosestVertex(nearestPoint, this.radius);
    return closestVertex;
  }
  return null;
}

    GridSnap.prototype.addOrRemoveMarker =
        function(vertex) {
  console.log(vertex);
  // var hash;
  ...
  // 	// TODO: replace this since it may break if octree is rebuilt or object
  // rotates (anything that may alter position of vertex)
  // 	// Should use a method that allows affine transformations on original
  // mesh?
  ...
  // this.scene.add(markerMesh);
        ...
        GridSnap.prototype.mouseUp =
            function(event) {
  console.log('mouseUp');
  if (!this.addMarkerOnMouseUp)
    return;
  ...
  this.addMarkerOnMouseUp = false;
            ...
            GridSnap.prototype.mouseDown =
                function(event) {
  console.log('mouseDown');
  this.addMarkerOnMouseUp = true
}

                GridSnap.prototype.mouseMove =
                    function(event) {
  ...
  if (!this.enabled || !this.hoverMesh)
    return;

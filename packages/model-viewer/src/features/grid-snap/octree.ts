import {BoxGeometry, MathUtils, Mesh, MeshBasicMaterial, Object3D, Vector3} from 'three';
...
Octree.prototype = {
  ...
  extend: function(octree) {
    var i, l, objectsData, objectData;
  ...
  rebuild: function() {
    var i, l, node, object, objectData, indexOctant, indexOctantLast,
        objectsUpdate = [];
  ...
  search: function(position, radius, organizeByObject, direction) {
    var i, l, node, objects, objectData, object, results, resultData,
  ...
  findClosestVertex: function(position, radius) {
    ...
    var object = search[0].object, vertices = search[0].vertices;
...
function OctreeObjectData(object, part) {
  ...
  // TODO: Added by KRISTIAN.
  // if (part instanceof Face) {
  //   // I have no idea what to do here. Face3 and Face4 are deprecated.
  // }
...
OctreeObjectData.prototype = {
  ...
  getFace3BoundingRadius: function(object, face) {
    var geometry = object.geometry || object,
        vertices = geometry.attributes.position.array, centroid = face.centroid,
  ...
  getFace4BoundingRadius: function(object, face) {
    var geometry = object.geometry || object,
        vertices = geometry.attributes.position.array, centroid = face.centroid,
        va = vertices[face.a], vb = vertices[face.b], vc = vertices[face.c],
        vd = vertices[face.d], centroidToVert = this.utilVec31FaceBounds,
        radius;
...
function OctreeNode(parameters) {
  ...
  this.position = parameters.position instanceof Vector3 ? parameters.position :
                                                           new Vector3();
  ...
  if (this.tree.scene) {
    ...
    this.visual.scale.set(
        this.radiusOverlap * 2, this.radiusOverlap * 2, this.radiusOverlap * 2);
...
OctreeNode.prototype = {
  ...
  reset: function(cascade, removeVisual) {
    var i, l, node, nodesIndices = this.nodesIndices || [],
  ...
  removeObject: function(object) {
    var i, l, nodesRemovedFrom, removeData;

    // cascade through tree to find and remove object

    removeData = this.removeObjectRecursive(
        object,
        {searchComplete: false, nodesRemovedFrom: [], objectsDataRemoved: []});
  ...
  grow: function() {
    var indexOctant, object, objectsExpand = [], objectsExpandOctants = [],
  ...
  split: function(objects, octants) {
    var i, l, indexOctant, object, node, objectsRemaining;
  ...
  branch: function(indexOctant) {
    var node, overlap, radius, radiusOffset, offset, position;
  ...
  expand: function(objects, octants) {
    var i, l, object, objectsRemaining, objectsExpand, indexOctant,
    ...
    if (this.tree.root.getDepthEnd() < this.tree.depthMax) {
      ...
      if (objectsExpand.length > 0) {
        ...
        // radius of parent comes from reversing overlap of this, unless overlap
        // percent is 0
  ...
  checkMerge: function() {
    ...
    // traverse up tree as long as node + entire subtree's object count is under
    // minimum
  ...
  checkContract: function() {
    var i, l, node, nodeObjectsCount, nodeHeaviest, nodeHeaviestObjectsCount,
  ...
  search: function(position, radius, objects, direction, directionPct) {
    ...
    // if intersects
    console.log('intersects bool', intersects);
    if (intersects === true) {
      // gather objects
  ...
  intersectSphere: function(position, radius) {
    var distance = radius * radius, px = position.x, py = position.y,
        pz = position.z;
